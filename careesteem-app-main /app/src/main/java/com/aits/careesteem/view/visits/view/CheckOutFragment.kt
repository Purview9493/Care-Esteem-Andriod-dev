...
//            val latestAllowed = plannedDateTime.plus(bufferDuration)
            // above line removed as per ios

            // Fixed buffer = 20 minutes (same as iOS)
            val bufferMinutes = 20L
            val earliestAllowed = plannedDateTime.minusMinutes(bufferMinutes)
            val latestAllowed = plannedDateTime.plusMinutes(bufferMinutes)
